/**
 * EvilEye — Vanilla JS port of the React Bits EvilEye component.
 * Uses OGL UMD bundle (loaded via standard script tag).
 */
(function () {
  'use strict';

  function initEvilEye(container, options = {}) {
    if (!container) return null;

    const OGL = window.OGL;
    if (!OGL) {
      console.warn('EvilEye: OGL library not found. Make sure ogl.umd.js is loaded.');
      return null;
    }
...
void main() {
  ...
  window.initEvilEye = initEvilEye;

  function mount() {
    const host = document.getElementById('evil-eye-host');
    if (!host) return;
    initEvilEye(host, {
      eyeColor:        '#a855f7',
      intensity:       1.5,
